...
from .base import AbstractSupervisedDBN as BaseAbstractSupervisedDBN
from .base import BaseModel
from .base import BinaryRBM as BaseBinaryRBM
from .base import UnsupervisedDBN as BaseUnsupervisedDBN
...
class BinaryRBM(BaseBinaryRBM, BaseTensorFlowModel):
    ...
    def _initialize_weights(self, weights):
        if weights:
            ...
            if self.activation_function == 'sigmoid':
                ...
                self.W = weight_variable(tf.compat.v1.truncated_normal, [self.n_hidden_units, self.n_visible_units], stddev)
    ...
    def _build_model(self, weights=None):
        ...
        self.random_uniform_values = tf.Variable(tf.compat.v1.random_uniform([self.batch_size, self.n_hidden_units]))
        ...
        for t in range(self.contrastive_divergence_iter):
            ...
            random_uniform_values = tf.Variable(tf.compat.v1.random_uniform([self.batch_size, self.n_hidden_units]))
...
class SupervisedDBNClassification(TensorFlowAbstractSupervisedDBN, ClassifierMixin):
    ...
    def _build_model(self, weights=None):
        ...
        self.cost_function = tf.reduce_mean(tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(logits=self.y, labels=tf.stop_gradient(self.y_)))
