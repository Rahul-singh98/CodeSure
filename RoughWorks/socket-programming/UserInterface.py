...
class Userinterface:
	...
	def submit(self):
		scriptwindow = Tk()
		scriptwindow.geometry('300x300')
		scriptwindow.wm_title(f'{self.script_name}')
		try:
			del(new_df)
			del(new_df1)
			del(new_df2)
		...
		label = Label(scriptwindow , text = '')
		label.pack()
		script_btn = Button(scriptwindow , text='Stop' ).pack()
		script_btn_exit = Button(scriptwindow , text = "Exit" , command = lambda : scriptwindow.destroy()).pack()

	    self.gap = self.gap1.get()
	    center = self.center1.get()
	    sst = self.sst1.get()
	    global ratio
	    global new_df3
	    new_df3 = new_df3.reset_index(drop=True)
	    # script_btn['text'] = 'Stop'
	    ...
	    if sst == 'Butterfly':
	        ...
	        self.fstart = self.start

	        self.generator = empty_box(script_btn ,self.n, self.fstart , self.end , self.gap )
	        # self.generator = empty_box(self.btn ,self.n, self.fstart , self.end , self.gap )

	        def stop():
	        	stopLoop(self.generator)
	        	script_btn['text'] = 'Start'
	        	# self.btn['text'] = 'Start'
	        	script_btn['command'] = self.submit
	        	# self.btn['command'] = self.submit

	        script_btn['command'] = stop

	        scriptwindow.mainloop()
	        # self.btn['command'] = stop 
